...
class Solution:
    def pacificAtlantic(self, heights: List[List[int]]) -> List[List[int]]:
        ...
        def dfs(x: int, y:int, solution, endfunc):
            ...
            if x + 1 < len(heights): neighbors.append((x+1,y))
            if y + 1 < len(heights[0]): neighbors.append((x,y+1))
        ...
        toAtlantic = set()
        last_x = len(heights) - 1
        last_y = len(heights[0]) - 1
        for x in range(len(heights)):
            for y in range(len(heights[0])):
        ...
        for x in range(len(heights)):
            for y in range(len(heights[0])):
                xy = (x,y)
                if xy in toAtlantic and xy in toPacific:
                    result.append([x,y])
        return result
...
class TestTemplate(unittest.TestCase): 
    ...
    def testCases(self):
        ...
        for heights, expected in param_list:
            with self.subTest():
                # arrange
                s = Solution()
                # act
